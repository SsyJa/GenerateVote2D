...
__global__ void preprocessCUDA(
	...
	const float3* dL_dvote3D,
	glm::vec3* dL_dmeans,
	glm::vec3* dL_dvote_offset,
	float* dL_dcolor,
	float* dL_dcov3D,
	float* dL_dsh,
	glm::vec3* dL_dscale,
	...
	float* dL_dopacity)
{
	...
	/*
	float3 v = votes[idx];
	float4 v_hom = transformPoint4x4(v, proj);
	float v_w = 1.0f/(v_hom.w + 0.0000001f);

	glm::vec3 dL_dvote3D;
	float vmul1 = (proj[0] * v.x + proj[4] * v.y + proj[8] * v.z + proj[12]) * v_w * v_w;
	float vmul2 = (proj[1] * v.x + proj[5] * v.y + proj[9] * v.z + proj[13]) * v_w * v_w;
	dL_dvote3D.x = (proj[0] * v_w - proj[3] * vmul1) * dL_dvote2D[idx].x + (proj[1] * v_w - proj[3] * vmul2) * dL_dvote2D[idx].y;
	dL_dvote3D.y = (proj[4] * v_w - proj[7] * vmul1) * dL_dvote2D[idx].x + (proj[5] * v_w - proj[7] * vmul2) * dL_dvote2D[idx].y;
	dL_dvote3D.z = (proj[8] * v_w - proj[11] * vmul1) * dL_dvote2D[idx].x + (proj[9] * v_w - proj[11] * vmul2) * dL_dvote2D[idx].y;
	*/
	//vote3d = ori+vote_offset so dL_dvote3D = dL_dvote_offset we do not propgate to ori(may be can try)
	dL_dvote_offset[idx] += glm::vec3(dL_dvote3D[idx].x, dL_dvote3D[idx].y, dL_dvote3D[idx].z);
	//dL_dvote_offset[idx] = {1,1,1};
...
template <uint32_t C, uint32_t V>
...
renderCUDA(
	...
	const float* __restrict__ vote_depths,
	const float* __restrict__ final_Ts,
	const float3* __restrict__ final_vote3d,
	const uint32_t* __restrict__ n_contrib,
	const float* __restrict__ dL_dpixels,
	const float* __restrict__ dL_dout_vote,
	const float* __restrict__ dL_ddistortion,
	const float* __restrict__ dL_invdepths,
	float3* __restrict__ dL_dmean2D,
	float3* __restrict__ dL_dvote3D,
...
)
{
	...
	const float3 v = inside? final_vote3d[pix_id] : zero_vec;
	//distortion
	float dL_dreg;
	...
	const int vote_contributor = inside ? n_contrib[pix_id + H * W] :0;
	if (inside)
	{	
		dL_dreg = dL_ddistortion[pix_id];
		for (int i = 0; i < C; i++)
			dL_dpixel[i] = dL_dpixels[i * H * W + pix_id];
		if(dL_invdepths)
		dL_invdepth = dL_invdepths[pix_id];
		for (int i = 0; i<V;i++)
			dL_dvotes[i] = dL_dout_vote[i * H * W + pix_id];
	}
	...
	const float dL_dvote_x = dL_dvotes[0];	
	const float dL_dvote_y = dL_dvotes[1];	
	// Gradient of pixel coordinate w.r.t. normalized 
	// screen-space viewport corrdinates (-1 to 1)
	const float ddelx_dx = 0.5 * W;
	const float ddely_dy = 0.5 * H;

	const float2 dL_dblendNDCvote = {dL_dvote_x * ddelx_dx, dL_dvote_y * ddely_dy};

	//3d space to ndc
	float3 dL_dblendvote3D = {0,0,0};

	if(inside)
	{
		if(vote_contributor>0)
		{
			float4 v_hom = transformPoint4x4(v, proj);
			float v_w = 1.0f/(v_hom.w + 0.0000001f);
			float vmul1 = (proj[0] * v.x + proj[4] * v.y + proj[8] * v.z + proj[12]) * v_w * v_w;
			float vmul2 = (proj[1] * v.x + proj[5] * v.y + proj[9] * v.z + proj[13]) * v_w * v_w;
			dL_dblendvote3D.x = (proj[0] * v_w - proj[3] * vmul1) * dL_dblendNDCvote.x + (proj[1] * v_w - proj[3] * vmul2) * dL_dblendNDCvote.y;
			dL_dblendvote3D.y = (proj[4] * v_w - proj[7] * vmul1) * dL_dblendNDCvote.x + (proj[5] * v_w - proj[7] * vmul2) * dL_dblendNDCvote.y;
			dL_dblendvote3D.z = (proj[8] * v_w - proj[11] * vmul1) * dL_dblendNDCvote.x + (proj[9] * v_w - proj[11] * vmul2) * dL_dblendNDCvote.y;
			dL_dblendvote3D.x = dL_dblendvote3D.x * (1.0f/static_cast<float>(vote_contributor));
			dL_dblendvote3D.y = dL_dblendvote3D.y * (1.0f/static_cast<float>(vote_contributor));
			dL_dblendvote3D.z = dL_dblendvote3D.z * (1.0f/static_cast<float>(vote_contributor));
		}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Only propagate offset ,do not alpha 
			if(T*(1-alpha)>0.01f){
				atomicAdd(&dL_dvote3D[global_id].x, dL_dblendvote3D.x );
				atomicAdd(&dL_dvote3D[global_id].y, dL_dblendvote3D.y );
				atomicAdd(&dL_dvote3D[global_id].z, dL_dblendvote3D.z ); //in world space
				// }
				//distortion
				float c_d = vote_depths[global_id];
				const float m_d = c_d; //far_n / (far_n - near_n) * (1 - near_n / c_d);
				const float dmd_dd = (far_n * near_n) / ((far_n - near_n) * c_d * c_d);
				// const float dL_dmd = 2.0f * (T * alpha) * (m_d * final_A - final_D) * dL_dreg;
				const float dL_dmd = 2.0f * (m_d * vote_contributor - final_D) * dL_dreg;
				float dL_dz = dL_dmd; //* dmd_dd; //in camera space
				atomicAdd(&dL_dvote3D[global_id].x, dL_dz * view[2]);
				atomicAdd(&dL_dvote3D[global_id].y, dL_dz * view[6]);
				atomicAdd(&dL_dvote3D[global_id].z, dL_dz * view[10]); 
			} 				
...
void BACKWARD::render(
	...
	const float* colors,
	const float* depths,
	const float* vote_depths,
	const float* final_Ts,
	const float3* final_vote3d,
	const uint32_t* n_contrib,
	const float* dL_dpixels,
	const float* dL_invdepths,
	const float* dL_dout_vote,
	...
	float* dL_dinvdepths)
{
	renderCUDA<NUM_CHANNELS, NUM_VOTES> << <grid, block >> >(
		ranges,
		point_list,
		W, H,
		bg_color,
